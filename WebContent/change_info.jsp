...
function btn_click(form){
	...
	// value値取得用
	var miss = false;
	var select_color = document.getElementById("color_select");
...
<body>
	...
	<form name="form" action="GearInfoUpdate" method="post" enctype="multipart/form-data">
	<table align="center" class="tbl">
	<c:forEach items="${closet}" var="gear">
	<tr><th>id</th><td>${gear.id}<input type="hidden" name="g_id" value="${gear.id}"></td></tr>
	<tr><th id="h_image">画像</th><td><input type="file" name="g_image" value=""></td></tr>
	<tr><th id="h_type">カテゴリー</th>
	<td><input type="search" name="g_type" autocomplete="on" size="15" value="${gear.type}" list="list_category">
					<!-- 候補を表示 -->
	...
	<td><select style="width:123px;" id="color_select" name="g_color" onchange="color_change(this)">
					<!-- 候補を表示 -->
					<option value="${gear.color}">${gear.color}</option>
	...
	<font color= "color:#ff0000" size="2">※必須</font></td></tr>
	<tr><th id="h_makers">ブランド</th><td><input type="text" name="g_makers" size="15" value="${gear.makers}">
	<font color= "color:#ff0000" size="2">※必須</font></td></tr>
	</c:forEach>
	</table>
	<div class="div_btn"><input type="button" value="変更" onclick="btn_click(this.form)"></div>
	</form>
	<hr width=85% size=2>
